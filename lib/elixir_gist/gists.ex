defmodule ElixirGist.Gists do
  ...
  """
  def create_gist(user, %Scope{} = scope, attrs) do
  gist =
    user
    |> Ecto.build_assoc(:gists) # assumes User has_many :gists
    |> Gist.changeset(attrs, scope)

  with {:ok, %Gist{} = gist} <- Repo.insert(gist) do
  ...
  """
  def create_saved_gist(user, %Scope{} = scope, attrs) do
  saved_gist =
    user
    |> Ecto.build_assoc(:saved_gists) # assumes User has_many :saved_gists
    |> SavedGist.changeset(attrs, scope)

  with {:ok, %SavedGist{} = saved_gist} <- Repo.insert(saved_gist) do
